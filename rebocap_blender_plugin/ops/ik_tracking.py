...
class REBOCAP_OT_create_tracking_nodes(bpy.types.Operator):
    ...
    def execute(self, context):
        ...
        for obj in list(bpy.data.objects):
            if obj.name == root_name or obj.name.startswith("Rebocap_"):
                bpy.data.objects.remove(obj, do_unlink=True)
                
        for mesh in list(bpy.data.meshes):
            if mesh.name.startswith("Rebocap_"):
...
class REBOCAP_OT_import_character(bpy.types.Operator):
    bl_idname = "rebocap.import_character"
    bl_label = "Import Character"
    bl_description = "Import Rebocap character (re-clicking refreshes it)"
    ...
    def execute(self, context):
        ...
        for obj in list(bpy.data.objects):
            if obj.get("rebocap_robot_character"):
                bpy.data.objects.remove(obj, do_unlink=True)
                character_exists = True
                
        # If it existed, we just deleted it. Since this is a toggle, we stop here.
        if character_exists:
            self.report({'INFO'}, "Rebocap character removed.")
            return {'FINISHED'}
        ...
        if addon_path:
            ...
            if os.path.exists(fbx_path):
                ...
                for obj in context.selected_objects:
                    obj["rebocap_robot_character"] = True
                ...
                if imported_armature:
                    ...
                    bone_mapping = {
                        'mixamorig:Hips':       'Rebocap_Pelvis',
                        'mixamorig:Spine':      'Rebocap_Spine1',
                        'mixamorig:Spine1':     'Rebocap_Spine2',
                        'mixamorig:Spine2':     'Rebocap_Spine3',
                        'mixamorig:Neck':       'Rebocap_Neck',
                        'mixamorig:LeftShoulder':  'Rebocap_L_Collar',
                        'mixamorig:RightShoulder': 'Rebocap_R_Collar',
                        'mixamorig:LeftArm':    'Rebocap_L_Shoulder',
                        'mixamorig:RightArm':   'Rebocap_R_Shoulder',
                        'mixamorig:LeftForeArm':  'Rebocap_L_Elbow',
                        'mixamorig:RightForeArm': 'Rebocap_R_Elbow',
                        'mixamorig:LeftHand':   'Rebocap_L_Wrist',
                        'mixamorig:RightHand':  'Rebocap_R_Wrist',
                        'mixamorig:LeftUpLeg':  'Rebocap_L_Hip',
                        'mixamorig:RightUpLeg': 'Rebocap_R_Hip',
                        'mixamorig:LeftLeg':    'Rebocap_L_Knee',
                        'mixamorig:RightLeg':   'Rebocap_R_Knee',
                        'mixamorig:LeftFoot':   'Rebocap_L_Ankle',
                        'mixamorig:RightFoot':  'Rebocap_R_Ankle',
                    }

                    # Disconnect all bones so each can be positioned independently
                    bpy.context.view_layer.objects.active = imported_armature
                    bpy.ops.object.mode_set(mode='EDIT')
                    for ebone in imported_armature.data.edit_bones:
                        ebone.use_connect = False
                    bpy.ops.object.mode_set(mode='OBJECT')
                    
                    stretch_targets = {
                        'mixamorig:Hips':       'Rebocap_Spine1',
                        'mixamorig:Spine':      'Rebocap_Spine2',
                        'mixamorig:Spine1':     'Rebocap_Spine3',
                        'mixamorig:Spine2':     'Rebocap_Neck',
                        'mixamorig:LeftUpLeg':  'Rebocap_L_Knee',
                        'mixamorig:RightUpLeg': 'Rebocap_R_Knee',
                        'mixamorig:LeftLeg':    'Rebocap_L_Ankle',
                        'mixamorig:RightLeg':   'Rebocap_R_Ankle',
                        'mixamorig:LeftShoulder': 'Rebocap_L_Shoulder',
                        'mixamorig:RightShoulder':'Rebocap_R_Shoulder',
                        'mixamorig:LeftArm':    'Rebocap_L_Elbow',
                        'mixamorig:RightArm':   'Rebocap_R_Elbow',
                        'mixamorig:LeftForeArm':'Rebocap_L_Wrist',
                        'mixamorig:RightForeArm':'Rebocap_R_Wrist',
                    }

                    # 1. Place bone position to the joint, and stretch to next joint
                    for bname, empty_name in bone_mapping.items():
                        pbone = imported_armature.pose.bones.get(bname)
                        if not pbone:
                            continue
                            
                        # Prevent STRETCH_TO scale from propagating to children
                        if hasattr(pbone.bone, 'inherit_scale'):
                        ...
                        if target_obj:
                            # Position: Copy Location (World -> World)
                            cl = pbone.constraints.new('COPY_LOCATION')
                            cl.target = target_obj
                            cl.target_space = 'WORLD'
                            cl.owner_space = 'WORLD'
                            
                        stretch_target_name = stretch_targets.get(bname)
                        if stretch_target_name:
                            st_obj = bpy.data.objects.get(stretch_target_name)
                            if st_obj:
                                st = pbone.constraints.new('STRETCH_TO')
                                st.target = st_obj
                                st.volume = 'NO_VOLUME'
                                st.rest_length = pbone.bone.length

                    # Apply uniform 3-axis scale to Neck and Head
                    neck_bone = imported_armature.pose.bones.get('mixamorig:Neck')
                    if neck_bone:
                        scale_factor = rebocap.sk_neck_head / 15.0
                        neck_bone.scale = (scale_factor, scale_factor, scale_factor)
                        
                    # Apply uniform 3-axis scale to Foot
                    # User requested: 30cm foot = 1.0 scale
                    foot_scale = rebocap.sk_foot / 30.0
                    left_foot = imported_armature.pose.bones.get('mixamorig:LeftFoot')
                    right_foot = imported_armature.pose.bones.get('mixamorig:RightFoot')
                    if left_foot: left_foot.scale = (foot_scale, foot_scale, foot_scale)
                    if right_foot: right_foot.scale = (foot_scale, foot_scale, foot_scale)

                if getattr(context.scene, 'rebocap_debug_log', False):
                    print(f"[Rebocap IK] Generated 24 Tracking Nodes.")
                    print(f"[Rebocap IK] Applied STRETCH_TO constraints with Rest_Length preservation.")
                    print(f"[Rebocap IK] Set initial Neck uniform scale to {scale_factor:.3f} (base 15.0)")
                    print(f"[Rebocap IK] Set initial Foot uniform scale to {foot_scale:.3f} (base 30.0)")

                self.report({'INFO'}, "Rebocap character imported. Re-click to refresh.")
                return {'FINISHED'}
            else:
                self.report({'ERROR'}, "Character FBX not found in assets.")
                return {'CANCELLED'}
        return {'CANCELLED'}
