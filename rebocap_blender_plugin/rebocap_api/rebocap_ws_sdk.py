...
try:
    ...
    elif py_version.startswith('3.13'):
        from .py313 import rebocap_ws_sdk_ext
    elif py_version.startswith('3.14'):
        from .py314 import rebocap_ws_sdk_ext
    else:
        raise RuntimeError(f"Unsupported Python version: {sys.version.split()[0]}. Supported: 3.6-3.14")
except Exception as e:
    pyd_load_error = str(e)
    # Create fallback mock module to prevent hard crash during import
    class DummyExtModule:
        pass
    rebocap_ws_sdk_ext = DummyExtModule()

my_pyd_module = rebocap_ws_sdk_ext
